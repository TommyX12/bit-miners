...
public class MapData : IGridCollidable {
    ...
    public List<UnitSpawnPoint> UnitSpawnPoints = new List<UnitSpawnPoint>();
    ...
    public void GenerateFromParam() {
        ...
        // enemy spawning
        this.UnitSpawnPoints.Clear();
        Unit enemyPrefab = EnemyManager.Current.GetPrefabFromString(Util.RandomSelectChance(biome.EnemySpawnTypeChance));
        if (enemyPrefab != null) {
            int spawnNumber = Util.RandomSelectChance(biome.EnemySpawnNumberChance);
            for (int i = 0; i < spawnNumber; i++) {
                this.UnitSpawnPoints.Add(new UnitSpawnPoint(enemyPrefab));
            }
        }
    }
}
